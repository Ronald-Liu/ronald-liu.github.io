...
DHOSTS = ['2001:4860:4860::8888', # remote dns server address list
	'8.8.8.8',
	'8.8.4.4',
	#'156.154.71.1',
	#'208.67.222.222',
...
def QueryDNS(server, port, querydata):
    ...
    if ':' in server:
	    addrP = socket.AF_INET6
    else:
	    addrP = socket.AF_INET
    try:
        s = socket.socket(addrP, socket.SOCK_STREAM)
        s.settimeout(TIMEOUT) # set socket timeout
	if addrP == socket.AF_INET6:
		s.connect((server, int(port), 0, 0))
	else: 
...
def configResolv(confPath='/etc/resolv.conf'):
    fio = open(confPath, 'r')
    lst = fio.readlines()
    if 'nameserver 127.0.0.1' in lst:
        fio.close()
        return
    fio.close()

    fio = open(confPath, 'w')
    lst.insert(0, 'nameserver 127.0.0.1\n')
    fio.writelines(lst)
    fio.close()
